...
zjednodušuje konfiguraci nativních závislostí a~nástroje
\texttt{expo prebuild} a~\texttt{expo run:*} umožňují generovat nativní
projekty jak lokálně, tak v~CI/CD pipeline na GitHub Actions. Expo SDK
navíc poskytuje stabilní přístup k~nativním modulům -- včetně integrace
s~rozhraními Health Connect~\cite{healthconnect2024} a~HealthKit~\cite{healthkit2024}.

\subsection{Next.js pro webovou vrstvu}

Doplňková webová vrstva systému byla implementována pomocí frameworku
Next.js nad knihovnou React. Framework byl zvolen kvůli podpoře
hybridního renderingu: veřejné landing page a~stránka stahování mohou
být generovány staticky s~pravidelnou revalidací, zatímco administrátorské
routy zůstávají dynamické a~pracují se stavovou autentizací založenou
na cookies. Vestavěné route handlery současně umožňují obsluhovat
stabilní URL pro nejnovější i~archivní mobilní buildy bez potřeby
samostatné distribuční služby.
...
Celá serverová a~webová vrstva je kontejnerizována pomocí nástroje Docker
...
(API server, webový frontend a~databáze) v~jediném konfiguračním souboru,
