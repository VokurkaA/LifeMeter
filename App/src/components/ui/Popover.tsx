...
import { Modal, Pressable, View, type LayoutChangeEvent, useWindowDimensions } from 'react-native';
...
export interface PopoverTriggerProps extends React.ComponentPropsWithoutRef<typeof Pressable> {
  ...
  containerClassName?: string; // classes for the internal wrapper when asChild
}
...
function PopoverTrigger({
  ...
  if (asChild && React.isValidElement(children)) {
    return (
      <View ref={wrapperRef} collapsable={false} onLayout={measure} className={containerClassName}>
...
function PopoverContent({
  ...
  return (
    <Modal visible={open} transparent animationType="fade" onRequestClose={() => setOpen(false)}>
      <Pressable
        className={overlayClassName ?? 'flex-1 bg-popover/20'}
        onPress={() => setOpen(false)}
      >
        <View className="flex-1">
          <View
            pointerEvents="box-none"
            style={[{ position: 'absolute', top, left, maxWidth: screenW - 16 }]}
          >
            <View
              onLayout={(e) =>
                setContentSize({
