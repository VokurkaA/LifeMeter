...
import SleepScreen from '@/screens/app/sleep/Index.scree'; // Preserving typo from file structure
...
function TabIcon({ icon, name, showName, active, onPress, activeColor, inactiveColor }: TabIconProps) {
    ...
    const styledIcon = React.isValidElement(icon)
        ? React.cloneElement(icon as React.ReactElement<{ color?: string; size?: number; strokeWidth?: number }>, {
            color: color,
            size: 28,
    ...
    return (
        <Pressable onPress={onPress} className="items-center justify-center gap-1 mt-3">
            {styledIcon}
            {showName && (
                <Text className={`${active ? 'text-foreground' : 'text-muted'} text-xs`}>{name}</Text>
            )}
        </Pressable>
    );
}
        ...
        <Popover>
            <Popover.Trigger asChild>
                <Pressable className="absolute bottom-0 translate-x-1/2 -translate-y-2 right-1/2">
                    <View className="items-center justify-center h-20 rounded-full aspect-square bg-field">
