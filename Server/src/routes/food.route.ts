...
foodRouter.get("/search", pagination(), async (c) => {
  ...
  if ((!rawName || rawName.trim() === "") && !gtin) {
    return c.json(
      { error: "Either 'name' or 'gtin' query parameter is required" },
      400,
    );
  }
  if (rawName && gtin) {
  ...
  if (rawName) {
    try {
      ...
      const { rows, total } = await foodService.getFoodByName(
        decoded,
        paginationProps,
      );
      return c.json({
      ...
      log.error(`Failed to search food by name: ${rawName}`, {
        error: e,
      });
  ...
  try {
    ...
    log.error(`Failed to search food by GTIN: ${gtin}`, {
      error: e,
    });
    return c.json({ error: "Internal server error" }, 500);
  }
...
foodRouter.get("/", pagination(), async (c) => {
  try {
    ...
    log.error("Failed to fetch all foods", {
      error: e,
    });
    return c.json({ error: "Internal server error" }, 500);
  }
...
foodRouter.get("/:id", async (c) => {
  ...
  try {
    ...
    log.error(`Failed to get food by ID: ${id}`, {
      error: e,
    });
    return c.json({ error: "Internal server error" }, 500);
  }
