...
export const userSleepRouter = new Hono<{
...
userSleepRouter.post("/start", async (c) => {
  ...
  try {
    ...
    log.error("Error starting sleep", {
      error: e,
    });
...
userSleepRouter.post("/new", async (c) => {
  ...
  try {
    const entry = await sleepService.addSleepEntry(
      userId,
      startAt,
      endAt,
      note,
    ...
    if (status === 500) {
      log.error("Error creating new sleep entry", {
        error: e,
      });
    }
...
userSleepRouter.get("/", async (c) => {
  ...
  try {
    ...
    log.error("Error fetching sleep entries", {
      error: e,
    });
...
userSleepRouter.get("/:id", async (c) => {
  ...
  try {
    ...
    if (status === 500) {
      log.error(`Error fetching sleep entry ${sleepEntryId}`, {
        error: e,
      });
    }
...
userSleepRouter.patch("/:id", async (c) => {
  ...
  try {
    ...
    const startAt: string =
      body?.startAt ?? (existing.sleep_start as unknown as string);
    const endAt: string | null =
      (Object.prototype.hasOwnProperty.call(body, "endAt")
        ? body.endAt
        : existing.sleep_end) ?? null;
    const note: string | null =
      (Object.prototype.hasOwnProperty.call(body, "note")
        ? body.note
        : existing.note) ?? null;

    const updated = await sleepService.alterSleepEntry(
      userId,
      sleepEntryId,
      startAt,
      endAt,
    ...
    if (status === 500) {
      log.error(`Error updating sleep entry ${sleepEntryId}`, {
        error: e,
      });
    }
...
userSleepRouter.delete("/:id", async (c) => {
  ...
  try {
    ...
    if (status === 500) {
      log.error(`Error deleting sleep entry ${sleepEntryId}`, {
        error: e,
      });
    }
